...
const Header = () => {
  ...
  const headerClasses = `
    ...
    ${isScrolled
      ? 'h-16 sm:h-20 bg-[#1a2e24]/95 shadow-lg backdrop-blur-lg'
      : 'h-20 sm:h-24 bg-gradient-to-b from-black/50 to-transparent'
    }
  `;

  const linkClasses = (path) => {
    const isActive = location.pathname === path;
    const baseStyle = 'px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-sm sm:text-base font-medium transition-all duration-300';
  ...
  const bookNowButtonClasses = `
    px-4 sm:px-6 py-2 sm:py-2.5 rounded-full font-bold text-sm sm:text-base
    ...
    <>
      <header className={headerClasses}>
        <nav className="container mx-auto px-3 sm:px-4 md:px-6 lg:px-8 flex justify-between items-center h-full">
          ...
          <div className="flex-shrink-0">
            <Link to="/">
              <img
                ...
                className={`w-auto transition-all duration-300 ease-in-out ${isScrolled ? 'h-24 sm:h-28 md:h-32' : 'h-28 sm:h-36 md:h-40'}`}
              />
            </Link>
          </div>
          ...
          {/* Desktop Book Now Button - Now links to contact-us */}
      ...
      >
        <div className="flex flex-col h-full items-center justify-center p-6 sm:p-8">
          <ul className="flex flex-col items-center space-y-6 sm:space-y-8 text-center">
            {navLinks.map((link) => (
              <li key={link.path}>
                <Link
                  to={link.path}
                  className={`text-2xl sm:text-3xl font-medium transition-colors ${location.pathname === link.path ? 'text-[#f59e0b]' : 'text-white'}`}
                >
                  {link.label}
                </Link>
              </li>
          ...
          {/* Mobile Book Now Button - Now links to contact-us */}
          <div className="mt-10 sm:mt-12 w-full max-w-xs px-4">
            <Link
              to="/contact-us"
              className="block w-full text-center bg-[#f59e0b] text-[#1a2e24] px-6 sm:px-8 py-3 sm:py-4 text-base sm:text-lg rounded-full font-bold transition-all duration-300 hover:bg-[#fbbf24] active:scale-95"
            >
              Book Now
            </Link>
          </div>
